...
/** A UI attachment that retains the original browser file for direct uploads. */
...
export const PromptInput = ({
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = useCallback(
    async (event) => {
      ...
      try {
        // Preserve locally selected files for direct multipart uploads. Attachments
        // supplied without an original File retain the existing blob-URL fallback.
        const convertedFiles: PromptInputFile[] = await Promise.all(
          files.map(async ({ sourceFile, ...item }) => {
            if (sourceFile) {
